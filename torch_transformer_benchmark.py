...
The default thresholds are atol=0.002 and rtol=0.02 (2%), matching the task's
stated contract (relative error < 0.02, abs error < 0.002).
"""
...
from optimized_transformer import OptimizedTransformerMixin
...
# ---------------------------------------------------------------------------
# The optimized forward lives in optimized_transformer.py. It is mixed in ahead
# of BaselineTransformer, so it overrides only forward(), adds no parameters and
# registers no buffers -- the state dict stays byte-identical and
# copy_model_weights(strict=True) keeps working. See that module's docstring for
# the techniques it applies and the TJ_* ablation toggles.
...
def benchmark_models(
    ...
    # baseline=None is the --optimized-only mode: some shapes (notably the
    # official shape #14, B=32 H=16 S=100000) cannot run the reference at all,
    # because its explicit [B, H, S, S] score tensor is 20 TB. Timing the
    # optimized path alone is the only measurement available there.
    if baseline is None:
        ...
        print(
            ...
            f"throughput="
            f"{tokens_per_call * 1000.0 / optimized_result.median_ms:.2f} token/s"
        )
        print("speedup  : n/a (baseline not runnable at this shape)")
        return
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "skip the accuracy comparison and time only the optimized model. "
            "For shapes whose reference implementation cannot run at all -- the "
            "official shape #14 needs a 20 TB score tensor in the baseline."
        ),
    )
...
def main() -> int:
    ...
    if args.optimized_only:
        # Left on the host. At shape #14 the input and output tensors alone are
        # ~26 GB of device memory, so the unusable reference must not also take
        # a share of it.
        baseline = None
    else:
    ...
    if baseline is None:
        print("\n=== Accuracy check ===")
        print("skipped: --optimized-only, no runnable reference at this shape")
